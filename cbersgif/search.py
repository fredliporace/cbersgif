...
class Search:  # pylint: disable=too-few-public-methods
    ...
    def is_online(self) -> bool:
        """Return True if search service is online."""
        try:
...
class StacSearch(Search):
    ...
    def search(  # pylint: disable=too-many-arguments
        ...
        req = requests.post(
            self.search_url, json=params, headers={"Content-Type": "application/json"}
        )

        if req.status_code is not requests.codes.ok:  # pylint: disable=no-member
            raise RuntimeError(
                "Service returned %d code, msg: %s" % (req.status_code, req.text)
            )
        ...
        assert len(res) < limit, "Possible truncation on the number of returned scenes"
