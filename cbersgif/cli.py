...
@click.option("--res", type=int, default=20, help="Output Resolution")
@click.option(
    "--bands",
    type=str,
    default="7,6,5",
...
@click.option(
    "--saveintermediary/--nosaveintermediary",
    default=False,
    help="Save intermediary files as bmp",
)
@click.option(
    "--max_images", type=int, default=100, help="Maximum number of images to be used"
)
@click.option(
    "--singleenhancement/--nosingleenhancement",
    default=False,
...
@click.option(
    "--contrast_factor", type=float, default=1.0, help="Contrast enhancement factor"
)  # pylint: disable=too-many-arguments, too-many-locals
@click.option(
    "--brightness_factor", type=float, default=1.0, help="Brightness enhancement factor"
)
@click.option(
    "--duration", type=float, default=0.5, help="Duration of each frame, in seconds"
)
@click.option(
    "--taboo_index",
    type=str,
...
@click.option(
    "--stac_endpoint",
    "-s",
    type=str,  # pylint: disable=too-many-locals,too-many-arguments
...
def main(
    ...
    for scene_no, scene in enumerate(scenes):
        ...
        print(scene)
        s3_key = "s3://{bucket}/{dir}".format(bucket=s3_bucket, dir=scene["key"])
        print(s3_key)
        ...
        for band_no, band in enumerate(rgb):
            ...
            matrix = utils.get_frame_matrix(
                s3_key, band, scene, aoi_bounds, width, height
            )
            ...
            if (not images or not singleenhancement) and enhancement:
                p_min_value[band_no], p_max_value[band_no] = np.percentile(
                    matrix[matrix > 0], (p_min, p_max)
                )
                print(
                    "{}, {}-{}, {}-{}".format(
                        band_no, p_min, p_max, p_min_value, p_max_value
                    )
                )
            ...
            if enhancement:
                matrix = np.where(
                    ...
                    utils.linear_rescale(
                        matrix,
                        in_range=[int(p_min_value[band_no]), int(p_max_value[band_no])],
                        out_range=[1, 255],
                    ),
                    0,
                )
