...
def test_stac_to_aws_sat_api():
    ...
    scene = stac_to_aws_sat_api(stac_id="CBERS_4_PAN5M_20181231_" "156_107_L2")
    assert scene["key"] == "CBERS4/PAN5M/156/107/CBERS_4_PAN5M_20181231_" "156_107_L2"
...
def test_search_stac_mode():
    ...
    assert len(result) >= 4
    assert result[0]["key"] == "CBERS4/MUX/151/126/CBERS_4_MUX_20150215_151_126_L2"
...
def test_search_date():
    ...
    assert len(result) >= 4
    result = search(
        sensor="MUX", path=100, row=100, start_date="2015-11-01", end_date="2018-09-29"
    )
    assert len(result) == 2
...
def test_search_remove_dupes():
    ...
    for index in range(0, len(result) - 1):
        assert (
            result[index]["acquisition_date"] != result[index + 1]["acquisition_date"]
        )
...
def test_lonlat_to_geojson():
    ...
    with open("tests/copacabana.json", "w") as fp_out:
        fp_out.write(result)
    res = diff_files("tests/copacabana.json", "tests/ref_copacabana.json")
    assert len(res) == 0, res

    result = lonlat_to_geojson(-43.182365, -22.970722, 10000)
    with open("tests/copacabana_10000.json", "w") as fp_out:
        fp_out.write(result)
    res = diff_files("tests/copacabana_10000.json", "tests/ref_copacabana_10000.json")
    assert len(res) == 0, res
...
def test_feat_to_bounds():
    ...
    bounds = feat_to_bounds(lonlat_to_geojson(-43.182365, -22.970722, 10000))
    assert all(
        math.isclose(l_1, l_2, abs_tol=0.0002)
        for l_1, l_2 in zip(
            bounds,
            (
                -4817038.8830492785,
